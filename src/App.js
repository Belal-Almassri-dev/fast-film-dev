...
import locationsData from './json_files/locations.js'
...
class App extends Component {
  // Call constructor to initialise variables
  constructor(props) {
    super(props);
    this.state = {
      newItem: "",
  ...
  // Function to call next button event
  handleNextClick() {
  ...
  // Function to call previous button event
  handlePreviousClick() {
    this.state.stepCount = this.state.stepCount - 1;
    // stepCount = stepCount--;
    this.setState({ stepCount: this.state.stepCount-- });
  ...
  // Function to mount local storage variable
  componentDidMount() {
    ...
    // let locationList = [];
    // fetch('../json_files/locations.json')
    //   .then(response => {
    //     return response.json();
    //   }).then(data => {
    //     console.log("data ---------- ", data);
    //     locationList = data.results.map((location) => {
    //       return location
    //     });
    ...
    //   .then((findresponse) => {
    //     console.log( "location data 2nd block ------------ " ,findresponse)
    //     // this.setState({
  ...
  // Function to unmount local storage variable
  ...
  // Function to save local storage variable in LocalStorage
  ...
  // Function to update local storage variable
  ...
  // Function to used for add local storage variable
  addItem() {
  ...
  // Function used for delete local storage variable
  deleteItem(id) {
    // copy current list of items
    const list = [...this.state.list];
  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        <StepDisplay stepCount={stepCount} updateInput={this.updateInput}/>
    ...
    <div className="container">
      ...
      <div className="row">
        <div className="form-check form-check-inline">
          <input type="radio" className="form-check-input" id="materialInline1" name="inlineMaterialRadiosExample" value="highlights_film" onChange={e => props.updateInput("film_type", e.target.value)} />
          <label className="form-check-label" htmlFor="materialInline1">HIGHLIGHTS FILM</label>
        </div>

        <div className="form-check form-check-inline">
          <input type="radio" className="form-check-input" id="materialInline2" name="inlineMaterialRadiosExample" value="animation" onChange={e => props.updateInput("film_type", e.target.value)} />
          <label className="form-check-label" htmlFor="materialInline2">ANIMATION</label>
        </div>
      </div>
      <div className="row">
        <div className="form-check form-check-inline">
          <input type="radio" className="form-check-input" id="materialInline3" name="inlineMaterialRadiosExample" value="taking_interview" onChange={e => props.updateInput("film_type", e.target.value)} />
          <label className="form-check-label" htmlFor="materialInline3">TALKING HEAD / INTERVIEW</label>
        </div>

        <div className="form-check form-check-inline">
          <input type="radio" className="form-check-input" id="materialInline4" name="inlineMaterialRadiosExample" value="request_callback" onChange={e => props.updateInput("film_type", e.target.value)} />
...
function StepTwo(props) {
  // const locationsList = this.state.location_list;
  return (
    <div className="container">
      ...
      <div className="row">
        <div className="form-check form-check-inline">
          ...
          <select name="location_name" className="form-control">
          <option value="">Search (Post code, building name)</option>
          {locationsData.map((item, key)=>
            <option key={key} value={item.oid}>{item.location_name}</option>
          )}            
          </select>
        </div>
      </div>
...
function StepDisplay(props) {
  ...
  if (stepCount == 0) {
    ...
    return <StepOne updateInput={props.updateInput} />;
  }
  else if (stepCount == 2) {
    return <StepTwo />;
  }
